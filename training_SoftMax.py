...
#        #for hidden layers
        for i in range(len(activations)-2):
            ...
            dZ[activations[-2-i] <=0] = 0 #ReLu backProp, using -2 becuase, last layer which is the output layer has been computed, we start from the layer before the last layer
            dZs.insert(0,dZ)
    ...
    def backProps(self,epochs,l_rate,mini_batch_size,training_data,test_data,reg_strength = 0.01):
...
test = backProp([28*28,800,10]) #setup the network size in [i,x,o], where i is the input size, o is the ouput size, x is the hidden layer size. Multiple X can be added so [i,x1,x2,xn,o] is possible
test.backProps(30,0.01,10000,training_data,test_data) 
